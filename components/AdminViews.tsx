...
export const CMSView: React.FC = () => {
    const [activeTab, setActiveTab] = useState<'home' | 'pages' | 'menu' | 'articles' | 'events' | 'changelog'>('home');
...
const EventsManager: React.FC = () => {
    const [events, setEvents] = useState<any[]>(db.events.getAll());

    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <h3 className="text-lg font-bold text-slate-800">Cerebro de Eventos (Virtual & Presencial)</h3>
                <button className="flex items-center gap-2 px-4 py-2 bg-cafh-blue text-white rounded-xl hover:bg-blue-900 transition-all shadow-sm text-sm font-bold">
                    <Plus size={18} />
                    CREAR EVENTO
                </button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {events.map(event => (
                    <div key={event.id} className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm hover:shadow-md transition-all">
                        <div className="flex justify-between items-start mb-4">
                            <div className={`w-14 h-14 rounded-2xl flex flex-col items-center justify-center text-white font-black ${event.color || 'bg-cafh-blue'}`}>
                                <span className="text-xl leading-none">{event.day}</span>
                                <span className="text-[10px] uppercase">{event.month}</span>
                            </div>
                            <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase ${event.type === 'Virtual' ? 'bg-purple-50 text-purple-600' : 'bg-green-50 text-green-600'
                                }`}>
                                {event.type}
                            </span>
                        </div>
                        <h4 className="font-bold text-slate-800 text-lg mb-2">{event.title}</h4>
                        <div className="space-y-2 mb-6">
                            ...
                            {event.zoomId && (
                                <div className="flex items-center gap-2 text-xs text-blue-600 font-bold bg-blue-50 p-2 rounded-lg">
                                    <Video size={14} />
                                    Zoom ID: {event.zoomId}
                                </div>
                            )}
                        </div>
                        <div className="flex gap-2 pt-4 border-t border-slate-50">
                            <button className="flex-1 py-2 bg-slate-50 text-slate-600 rounded-xl text-xs font-bold hover:bg-slate-100 transition-all">EDITAR</button>
